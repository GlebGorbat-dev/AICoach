...
class PyObjectId:
    ...
    def __get_pydantic_core_schema__(
        ...
        """Формирует схему Pydantic для валидации строкового ObjectId."""
    ...
    def __get_pydantic_json_schema__(
            cls, schema: core_schema.CoreSchema, hamdler: GetCoreSchemaHandler
    ) -> JsonSchemaValue:
        """Описывает JSON-схему ObjectId как строковый тип."""
        return {"type": "string"}

    @classmethod
    def validate(cls, value: str) ->ObjectId:
        """Проверяет строку и преобразует её в объект ObjectId."""
        if not ObjectId.is_valid(value):
            raise ValueError(f"Invalid ObjectId {value}")
        return ObjectId(value)

    def __getattr__(self, item):
        """Проксирует обращения к атрибутам напрямую к объекту ObjectId."""
        return getattr(self.__dict__["value"], item)

    def __init__(self, value: str = None):
        """Создаёт новый ObjectId или валидирует переданное значение."""
        if value is None:
            self.value = ObjectId(value)
        else:
...
class MongoBaseModel(BaseModel):
    ...
    def to_mongo(self) -> Dict[str, Any]:
        """Преобразует модель и вложенные объекты в словарь для сохранения в MongoDB."""
        def model_to_dict(model: BaseModel) -> Dict[str, Any]:
            """Рекурсивно собирает данные модели в словарь с учётом вложенных структур."""
            doc = {}
    ...
    def from_mongo(cls, data: Dict[str, Any]):
        """Создаёт экземпляр модели из документа MongoDB с восстановлением Enum."""

        def restore_enums(inst: Any, model_cls: Type[BaseModel]) -> None:
            """Рекурсивно преобразует сохранённые значения обратно в Enum."""
